...
export default function ImageAnimation({ images, device, socialItems }: Props) {
  ...
  return (
    <>
      {device === 'desktop' ? (
        <div class="relative w-[1200px] h-[701px] m-auto">
          {images.map((item, index) => (
            <Image
              src={item.image}
              key={index}
              class={`rounded-2xl h-[701px] w-full object-cover absolute top-0 right-0 ${
                currentImageIndex.value === index ? 'opacity-100' : 'opacity-0'
              }`}
              width={1200}
              height={701}
              loading="eager"
          ...
          <div class="hidden lg:block bg-white w-[496px] h-[308px] rounded-[30px] absolute -top-6 -left-6">
            <div class="bg-black w-[426px] h-[226px] rounded-2xl flex items-center justify-center">
              <div class="w-[379px] flex flex-col max-auto gap-4">
                <p class="text-2xl text-white font-medium leading-[34px] font-clash-display">
          ...
          <div class="absolute bottom-8 -left-10">
            <div class="flex flex-col gap-4">
              <ul class={`flex gap-4 lg:flex-col lg:items-start`}>
        ...
        <div class="w-full">
          {images.map((item, index) => (
            <Image
              ...
              key={index}
              class={`rounded-2xl h-auto w-full object-contain absolute top-0 right-0 ${
                currentImageIndex.value === index ? 'opacity-100' : 'opacity-0'
              }`}
              width={446}
              height={607}
